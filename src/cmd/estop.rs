...
fn build_resume_selector(
    ...
    domains: Vec<String>,
    tools: Vec<String>,
) -> anyhow::Result<security::ResumeSelector> {
    let selected =
...
fn build_engage_level(
    ...
    domains: Vec<String>,
    tools: Vec<String>,
) -> anyhow::Result<security::EstopLevel> {
...
pub(crate) async fn run(args: EstopArgs, config: &Config) -> anyhow::Result<()> {
    let EstopArgs {
        estop_command,
        level,
        domains,
